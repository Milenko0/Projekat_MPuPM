...
namespace FTN.Services.NetworkModelService.TestClient
{
    public class Program
    {
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (str == "4")
                    {
                        try
                        {
                            tgda.GetAcLineSegmentForMaxGID();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetAcLineSegmentForMaxGID failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
        ...
        private static void PrintMenu()
        {
            ...
            Console.WriteLine("\t4) Get AcLineSegment For MaxGID");
            Console.WriteLine("\t5) Test apply update");
