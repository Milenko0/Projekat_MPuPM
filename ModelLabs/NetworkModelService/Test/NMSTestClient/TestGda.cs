...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
    public class TestGda : IDisposable
    {
      ...
      /*  public List<long> GetAcLineSegmentForMaxGID()
        {
            ModelCode perLengthImpedanceModelCode = ModelCode.PERLENGTHIMPEDANCE;
            ModelCode acLineSegmentModelCode = ModelCode.ACLINESEGMENT;

            string message = "Getting AcLineSegment for PerLengthImpedance with the highest GID started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);

            XmlTextWriter xmlWriter = null;
            List<long> acLineSegmentIds = new List<long>();

            long perLengthImpedanceWithMaxGID = GetPerLengthImpedanceWithMaxGID();

            if (perLengthImpedanceWithMaxGID == 0)
            {
                message = "No PerLengthImpedance found.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                return acLineSegmentIds;
            }

            try
            {
                List<ModelCode> perLengthImpedanceProperties = modelResourcesDesc.GetAllPropertyIds(perLengthImpedanceModelCode);
                int iteratorId = GdaQueryProxy.GetRelatedValues(perLengthImpedanceWithMaxGID, perLengthImpedanceProperties, association.Type);
                int resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);

                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\AcLineSegmentForMaxGID_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;

                while (resourcesLeft > 0)
                {
                    List<ResourceDescription> rds = GdaQueryProxy.IteratorNext(resourcesLeft, iteratorId);

                    for (int i = 0; i < rds.Count; i++)
                    {
                        acLineSegmentIds.Add(rds[i].Id);
                        rds[i].ExportToXml(xmlWriter);
                        xmlWriter.Flush();
                    }
                ...
                message = "Getting AcLineSegment for PerLengthImpedance with the highest GID successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            {
                message = string.Format("Getting AcLineSegment for PerLengthImpedance with the highest GID failed.\n\t{0}", e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
        ...
        private long GetPerLengthImpedanceWithMaxGID()
        {
            ModelCode perLengthImpedanceModelCode = ModelCode.PERLENGTHIMPEDANCE;

            List<ModelCode> perLengthImpedanceProperties = modelResourcesDesc.GetAllPropertyIds(perLengthImpedanceModelCode);
            int iteratorId = GdaQueryProxy.GetExtentValues(perLengthImpedanceModelCode, perLengthImpedanceProperties);
            int resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);
        ...
        public List<ResourceDescription> GetAcLineSegmentForMaxGID()
        {
            string message = "Getting AcLineSegment for PerLengthImpedance with the highest GID started."; 
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                List<long> allAcLineSegment = GetExtentValues(ModelCode.ACLINESEGMENT);
                long highestAcLineSegmentId = allAcLineSegment.Max();

                List<long> allPerLengthImpedance = GetExtentValues(ModelCode.PERLENGTHIMPEDANCE);

                for (int i = 0; i < allPerLengthImpedance.Count; i++)
                {
                    rd = GetValues(allPerLengthImpedance[i]);
                    if (rd.Properties[13].PropertyValue.LongValue == highestAcLineSegmentId)
                        rdList.Add(rd);
                }

                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetAcLineSegmentForMaxGID_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                if (rdList.Count > 0)
                {
